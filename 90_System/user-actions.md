...
- [ ] n8nのブラウザ初回セットアップ(オーナーアカウント作成) | 2026-07-19 | [SNS投稿自動化/tasks.md](../20_Projects/SNS投稿自動化/tasks.md) MS1→次タスクの前提 | LAN内の任意の端末から `http://192.168.10.142:5678` を開くだけ。5分作業
- [ ] n8nでWebhook起動→SNS API呼び出しの最小ワークフローを1つ作成し、手動実行でテスト投稿が成功することを確認する | 2026-07-19 | [SNS投稿自動化/tasks.md](../20_Projects/SNS投稿自動化/tasks.md) MS1残タスク | n8n管理画面での操作が必要なためClaudeは実行不可。オーナーアカウント作成が前提。2026-W31週次レビューで台帳への記載漏れを発見し追記
