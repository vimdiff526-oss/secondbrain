...
- [ ] **(先行)Dockerのインストール**: `curl -fsSL https://get.docker.com | sudo sh` 実行後、`sudo usermod -aG docker sloasea` | 2026-07-19 | n8n起動の前提 | 2026-07-19の実機確認で docker コマンド自体が未インストールと判明。sudoパスワードが必要なためユーザー作業
- [ ] n8nコンテナを起動し管理画面にアクセス確認する | 2026-07-17 | [SNS投稿自動化/tasks.md](../20_Projects/SNS投稿自動化/tasks.md) MS1 | 手順は[実装メモ.md](../20_Projects/SNS投稿自動化/実装メモ.md)に完備。上のDockerインストール完了後、compose.yml配置→`docker compose up -d`→ブラウザ確認の約15分作業
